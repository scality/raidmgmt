...
#   4. Copy the output directory back to the project's testdata folder
#
# Prerequisites:
...
mkdir -p "${OUTPUT_DIR}/controllers/storcli2"
mkdir -p "${OUTPUT_DIR}/physicaldrives/show/storcli2"
mkdir -p "${OUTPUT_DIR}/physicaldrives/blink/storcli2"
mkdir -p "${OUTPUT_DIR}/physicaldrives/jbod/enable/storcli2"
mkdir -p "${OUTPUT_DIR}/physicaldrives/jbod/disable/storcli2"
mkdir -p "${OUTPUT_DIR}/logicalvolumes/show/storcli2"
mkdir -p "${OUTPUT_DIR}/logicalvolumes/create/storcli2"
mkdir -p "${OUTPUT_DIR}/logicalvolumes/delete/storcli2"
mkdir -p "${OUTPUT_DIR}/logicalvolumes/cacheoptions/storcli2"
mkdir -p "${OUTPUT_DIR}/logicalvolumes/migrate/storcli2"
...
# controllers/storcli2/all.json
# Command: storcli2 show all J
# Used by: adapter.showAll() → runner.Run(["show", "all"])
run_and_save \
    "All controllers overview" \
    "${OUTPUT_DIR}/controllers/storcli2/all.json" \
    show all

# controllers/storcli2/c0.json
...
run_and_save \
    ...
    "${OUTPUT_DIR}/controllers/storcli2/c0.json" \
    "${C}" show all

# controllers/storcli2/c5_invalid.json
# Command: storcli2 /c5 show all J
# Used by: error case when controller doesn't exist
run_and_save \
    "Invalid controller (expected failure)" \
    "${OUTPUT_DIR}/controllers/storcli2/c5_invalid.json" \
    "/c${INVALID_CONTROLLER_INDEX}" show all

# controllers/storcli2/c0_s12_UGood.json
...
# physicaldrives/show/storcli2/all.json
...
run_and_save \
    ...
    "${OUTPUT_DIR}/physicaldrives/show/storcli2/all.json" \
...
for eid in ${ENCLOSURE_IDS}; do
    for slot in ${SLOTS}; do
        run_and_save \
            ...
            "${OUTPUT_DIR}/physicaldrives/show/storcli2/e${eid}s${slot}.json" \
...
run_and_save \
    ...
    "${OUTPUT_DIR}/physicaldrives/show/storcli2/e${FIRST_ENCLOSURE}s${INVALID_SLOT}_invalid.json" \
...
run_and_save \
    ...
    "${OUTPUT_DIR}/physicaldrives/blink/storcli2/start.json" \
...
run_and_save \
    ...
    "${OUTPUT_DIR}/physicaldrives/blink/storcli2/stop.json" \
...
run_and_save \
    ...
    "${OUTPUT_DIR}/physicaldrives/jbod/enable/storcli2/fail.json" \
...
run_and_save \
    ...
    "${OUTPUT_DIR}/physicaldrives/jbod/disable/storcli2/fail.json" \
...
for vd_id in ${VD_IDS}; do
    run_and_save \
        ...
        "${OUTPUT_DIR}/logicalvolumes/show/storcli2/v${vd_id}.json" \
        "${C}/v${vd_id}" show all
done
...
run_and_save \
    "All virtual drives" \
    "${OUTPUT_DIR}/logicalvolumes/show/storcli2/all.json" \
    "${C}/vall" show all
...
run_and_save \
    ...
    "${OUTPUT_DIR}/logicalvolumes/show/storcli2/v${INVALID_VD_ID}_invalid.json" \
...
run_and_save \
    ...
    "${OUTPUT_DIR}/logicalvolumes/delete/storcli2/fail_vdNotExist.json" \
...
run_and_save \
    ...
    "${OUTPUT_DIR}/logicalvolumes/migrate/storcli2/fail.json" \
...
" 2>/dev/null || echo "")
    ...
    if [ -z "${SACRIFICE_DRIVE}" ]; then
        ...
        run_and_save \
            ...
            "${OUTPUT_DIR}/logicalvolumes/delete/storcli2/success.json" \
        ...
        run_and_save \
            ...
            "${OUTPUT_DIR}/controllers/storcli2/c0_s12_UGood.json" \
            "${C}" show all

        # physicaldrives/show/storcli2/e{EID}s{SLOT}_UGood.json
        ...
        run_and_save \
            ...
            "${OUTPUT_DIR}/physicaldrives/show/storcli2/e${SACRIFICE_EID}s${SACRIFICE_SLOT}_UGood.json" \
        ...
        # logicalvolumes/create/storcli2/success.json
        ...
        run_and_save \
            ...
            "${OUTPUT_DIR}/logicalvolumes/create/storcli2/success.json" \
...
" 2>/dev/null || echo "")
        ...
        if [ -n "${NEWEST_VD}" ]; then
            ...
            # logicalvolumes/cacheoptions/storcli2/success.json
            ...
            run_and_save \
                ...
                "${OUTPUT_DIR}/logicalvolumes/cacheoptions/storcli2/success_wrcache.json" \
                "${C}/v${NEWEST_VD}" set wrcache=WT

            run_and_save \
                "Set read cache on v${NEWEST_VD}" \
                "${OUTPUT_DIR}/logicalvolumes/cacheoptions/storcli2/success_rdcache.json" \
            ...
            run_and_save \
                ...
                "${OUTPUT_DIR}/logicalvolumes/cacheoptions/storcli2/success.json" \
        ...
        # logicalvolumes/create/storcli2/fail.json
        ...
        run_and_save \
            ...
            "${OUTPUT_DIR}/logicalvolumes/create/storcli2/fail.json" \
            "${C}" add vd raid0 "drives=${FIRST_ENCLOSURE}:${FIRST_SLOT}"

        # --- Step 8: Delete failure cases ---
        # logicalvolumes/delete/storcli2/fail_invalid.json
        # Command: storcli2 /c0/v299 delete J
        run_and_save \
            "Delete invalid VD (expected failure)" \
            "${OUTPUT_DIR}/logicalvolumes/delete/storcli2/fail_invalid.json" \
            "${C}/v299" delete
    ...
    echo "    - controllers/storcli2/c0_s12_UGood.json (controller with UGood drive)"
    echo "    - physicaldrives/show/storcli2/e{EID}s{SLOT}_UGood.json"
    echo "    - logicalvolumes/create/storcli2/success.json"
    echo "    - logicalvolumes/create/storcli2/fail.json"
    echo "    - logicalvolumes/delete/storcli2/success.json"
    echo "    - logicalvolumes/delete/storcli2/fail_invalid.json"
    echo "    - logicalvolumes/cacheoptions/storcli2/success.json"
    echo ""
fi
...
echo "  1. Copy the contents of ${OUTPUT_DIR}/ to:"
echo "     pkg/implementation/raidcontroller/megaraid/testdata/"
echo ""
